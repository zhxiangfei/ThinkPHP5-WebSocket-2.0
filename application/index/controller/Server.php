...
class Server extends Controller
{
    ...
    /**
     * 添加用户登陆信息
     * @param username headerimg login_time
     */
    public function addLogin($data)
    {
        ...
        try {
            ...
            foreach ($data['user_list'] as $key => $value) {
                // 先检测是否有该用户信息，如果有更新，没有新增
                if (Db::table('chat_user')->where(['username'=>$value['username']])->find()) {
                    
                    Db::table('chat_user')->where(['username'=>$value['username']])->update(['login_time'=>$value['login_time']]);
                }else{

                    Db::table('chat_user')->insert($value);
                }
            }
            Db::commit();
        } catch (Exception $e) {
    ...
    /**
     * 添加聊天记录
     * @param from(发送者username)  msg  headerimg
     */
    public function sendMsg($data)
    {
        ...
        // print_r($data);
        unset($data['type']);
        $res = Db::table('chat_msg')->insert($data);
        return $res;
    }
